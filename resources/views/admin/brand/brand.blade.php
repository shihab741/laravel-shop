  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <div class="row">
        ...
        <div class="col-md-4">
          <div align="right"><button class="btn btn-primary float-right page-header" data-toggle="modal" data-target="#exampleModal"><i class="fa fa-plus"></i> Add new brand</button></div>
        </div>
      </div>
    </section>
          ...
          <div class="box">
            ...
            <div class="box-body">
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tr>
                  <th>Serial no.</th>
                  <th>Brand name and description</th>
                  <th>Image</th>
                ...
                <tbody>
                       @php($i = 1)
                        @foreach($brands as $brand)
                            <tr>
                                <td>{{ $i++ }}</td>                               
                                <td><strong>{{ $brand->brand_name }}</strong><br> {{ $brand->brand_desc}}</td>
                                <td><img src="{{ asset('/') }}uploads/brand-images/{{$brand->brand_image}}" width="75px" height="75px"></td>
                                <td>{{ $brand->status == 1 ? 'Published' : 'Not published' }}</td>

                                <td>
                                    @if($brand->status == 1)
                                        <a href="{{route('unpublish-brand', ['id' => $brand->id])}}" class="btn btn-warning" title="Unpublish"><span><i class="fa fa-arrow-down"></i></span></a>
                                    @else
                                        <a href="{{route('publish-brand', ['id' => $brand->id])}}" class="btn btn-primary" title="Publish"><span><i class="fa fa-arrow-up"></i></span></a>
                                    @endif
                                    <a href="{{ route('brand.show', ['id' => $brand->id]) }}" class="btn btn-success"><span><i class="fa fa-edit"></i></span></a>
